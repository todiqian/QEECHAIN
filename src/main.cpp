...
unsigned int nStakeMinAge = 60 * 60 * 24 * 1;	// 最小资产龄：1天
unsigned int nStakeMaxAge = 60 * 60 * 24 * 100;	// 最大资产龄：100天
unsigned int nStakeTargetSpacing = 60;			// 区块与区块的间距60秒

int64 nChainStartTime = 1497776778; //区块链开始时间
int nCoinbaseMaturity = 30; //资产矿池成熟时间30s
...
CMedianFilter<int> cPeerBlockCounts(5, 0); // 其他节点拥有的区块数量
...
// 在给定的区块后，确保所有钱包都知道给定的交易
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // ppcoin: wallets need to refund inputs when disconnecting coinstake 当断开连接时，钱包需要退还转入金额
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
// notify wallets about a new best chain 通知钱包关于一个新的有效的区块链
...
// notify wallets about an updated transaction通知钱包关于更新的交易
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight == 1)
	{
        nSubsidy = 20000000 * COIN;	// 10 billion coins, that all pow coins初始总量
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    if(nHeight < YEARLY_BLOCKCOUNT){
        nRewardCoinYear = 16 * MAX_MINT_PROOF_OF_STAKE;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Jun 18, 2017, Putin Signs Treaty To Add Crimea To Map.";
        CTransaction txNew;
        ...
        printf("block.nNonce = %u \n", block.nNonce);
			block.print();
        assert(block.hashMerkleRoot == uint256("0xd3538a91d882df48730ea3e6dc6ed871cb1f214fa99a90b72b867be0ca362d11"));
