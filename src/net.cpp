...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    {
        // 我们应该逐步取消对这些网站的使用.如果我们需要
        // 替换，我们应该要求志愿者把这个简单。
        // 在Web服务器上打印客户机IP的PHP文件：
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "QEEChain " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. QEEChain is probably already running."), addrBind.ToString().c_str());
        else
