...
public:
    ...
    // Calculate in which "tried" bucket this entry belongs计算此条目所属的“试用”桶
    int GetTriedBucket(const std::vector<unsigned char> &nKey) const;

    // Calculate in which "new" bucket this entry belongs, given a certain source给定某个源，计算此条目所属的“新”桶
    int GetNewBucket(const std::vector<unsigned char> &nKey, const CNetAddr& src) const;

    // Calculate in which "new" bucket this entry belongs, using its default source计算此项所属的“新”桶，使用缺省源
    int GetNewBucket(const std::vector<unsigned char> &nKey) const
    {
        return GetNewBucket(nKey, source);
    }

    // Determine whether the statistics about this entry are bad enough so that it can just be deleted确定有关此项的统计信息是否足够糟糕，以便它可以被删除
    bool IsTerrible(int64 nNow = GetAdjustedTime()) const;

    // Calculate the relative chance this entry should be given when selecting nodes to connect to计算在选择要连接的节点时该条目的相对概率
...
// total number of buckets for tried addresses尝试地址的桶总数
